...
def load_data(file_path):
    ...
    daily_data = data.parse('Conso_jour')  # Consommation 
    Station = data.parse('Station')  # Consommation 
    return daily_data, Station

# Charger les données
file_path = "tableau de bord Wit.xlsx"
daily_data, Station = load_data(file_path)

# Page principale
st.title("Station")
st.write("Visualisation volumes et analyses station")
...
filtered_data = filtered_data[["Jour", "Consomation eau général","Station pre-traitement","Entrée Bassin","Sortie Bassin"]]
...
for var, color, unit in zip(variables, colors, units):
    fig.add_trace(go.Scatter(
        ...
        y=filtered_data[var] ,  # Ajuster les valeurs en fonction de l'échelle
        mode="lines+markers",
        name=f"{var} ({unit})",
        line=dict(color=color),
        yaxis="jours"  # Associer à l'axe principal ou secondaire
    ))
